# 현재가가 매수가의 -1.5% 이하로 떨어지면 손절 매도가 실행됩니다.
# 손절 매도 시 로그에 상세한 정보가 출력됩니다.
# Streamlit 대시보드의 거래 내역에 손절 거래가 표시됩니다.
# MACD 0 이하에서만 매수합니다.
...
def check_buy_conditions(conn):
    ...
    try:
        ...
        if not last_trade:
            print("거래 내역이 없습니다.")
            return False
        if last_trade[0] != 'SELL' and last_trade[0] != 'SELL_STOP' and last_trade[0] != 'INITIAL':
            return False
        ...
        # 디버깅을 위한 로그
        print("\n=== 매수 조건 체크 ===")
        print(f"MACD 값들: {[f'{x:.2f}' for x in recent_macd]}")
        print(f"하락 후 변곡점 체크:")
        print(f"  - 첫 하락: {recent_macd[0]:.2f} -> {recent_macd[1]:.2f}")
        print(f"  - 계속 하락: {recent_macd[1]:.2f} -> {recent_macd[2]:.2f}")
        print(f"  - 첫 상승: {recent_macd[2]:.2f} -> {recent_macd[3]:.2f}")
        print(f"  - 두번째 상승: {recent_macd[3]:.2f} -> {recent_macd[4]:.2f}")
...
def check_sell_conditions(conn, buy_price):
    ...
    try:
        ...
        if current_price <= buy_price * 0.985:
            print(f"손절 조건 발생: 매수가 {buy_price:,.0f}원, 현재가 {current_price:,.0f}원 (손실률: {((current_price - buy_price) / buy_price * 100):,.2f}%)")
            return "STOP_LOSS"
...
def execute_trade(conn, trade_type, current_price, balance, buy_price=None, is_stop_loss=False):
    """거래 실행 및 기록"""
    c = conn.cursor()
    now = datetime.now()
    ...
    try:
        if trade_type == 'BUY':
            ...
            if last_trade:
                ...
                sell_type = 'SELL_STOP' if is_stop_loss else 'SELL'
                
                c.execute('''
                    INSERT INTO trades 
                    (timestamp, type, price, amount, fee, balance, profit, profit_rate)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (now, sell_type, float(current_price), float(amount), 0.0, 
                      float(sell_amount), float(profit), float(profit_rate)))
                
                if is_stop_loss:
                    print(f"손절 매도: {float(current_price):,.0f}원 - 수익: {float(profit):,.0f}원 ({float(profit_rate):+.2f}%)")
                else:
                ...
                if price_data:
                    ...
                    try:
                        if (last_trade_type != 'BUY' and check_buy_conditions(conn)):
                            ...
                            sell_condition = check_sell_conditions(conn, buy_price)
                            if sell_condition == "STOP_LOSS":
                                balance = execute_trade(conn, 'SELL', current_price, balance, buy_price, True)
                                last_trade_type = 'SELL'
                                buy_price = None
                                print(f"손절 매도 신호 발생 및 실행 완료 - 현재가: {current_price:,.0f}원")
                            elif sell_condition == "PROFIT":
                                balance = execute_trade(conn, 'SELL', current_price, balance, buy_price, False)
                                last_trade_type = 'SELL'
                                buy_price = None
                                print(f"수익 매도 신호 발생 및 실행 완료 - 현재가: {current_price:,.0f}원")
                    except Exception as e:
