# 매수시 상승종목 비율에 따라 30,50,100% 매수하도록 수정함
# 현재가, 진입가격 표시됨
...
def fetch_price_data():
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 이전 분봉 데이터 가져오기 (2개의 봉을 가져와서 더 최신 데이터 사용)
...
def trading_loop():
    ...
    try:
        ...
        # trading_loop 함수 내부의 while True 부분
        while True:
            try:
                # 현재 시간이 1분이 되기를 기다림
                now = datetime.now()
                if now.second < 58:  # 58초부터 데이터 요청 시작
