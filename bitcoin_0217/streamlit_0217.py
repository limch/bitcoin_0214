        ...
        with col1:
            ...
            with get_connection() as conn:
                ...
                try:
                    ...
                    # 마켓 데이터 조회 (rising_count와 total_count도 함께 가져오기)
                    df_market = pd.read_sql("""
                        SELECT timestamp, rising_rate, rising_count, total_count
                        FROM market_data
                        ORDER BY timestamp DESC LIMIT 4320
                    """, conn)
                            ...
                            if macd is not None and signal is not None:
                                ...
                                fig.add_trace(
                                    go.Bar(
                                        ...
                                        marker=dict(
                                            color=colors,
                                            opacity=0.6  # opacity를 0.6으로 설정
                                        ),
                                        width=60000,  # 막대 너비 1분으로 설정
                                    ),
                                    row=1, col=1,
                                    secondary_y=False
                                )
                                ...
                                fig.update_yaxes(
                                    ...
                                    col=1,
                                    secondary_y=False,
                                    range=[min(df_prices['MACD_Hist'].min(), df_prices['MACD'].min()) * 1.2,
                                        ...
                                        for idx, row in sells.iterrows():
                                            ...
                                            fig.add_trace(
                                                go.Scatter(
                                                    ...
                                                    hovertemplate=f'{trade_type}: %{{y:,.0f}}원<br>수익률: {row["profit_rate"]:+.2f}%',
                                                    showlegend=True
                                                ),
                                                row=1, col=1,
                                                secondary_y=True
                                ...
                                fig.update_yaxes(title_text="BTC/KRW", row=1, col=1, secondary_y=True)
                                fig.update_yaxes(title_text="상승비율(%)", row=2, col=1)
