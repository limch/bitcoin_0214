                ...
                try:
                    ...
                    if not last_trade.empty:
                        ...
                        if trade_type == 'BUY':
                            ...
                            st.markdown("### 🟢 매수중")
                            st.write(f"진입시간: {format_datetime(entry_time)}")
                            st.write(f"진입가격: {format_number(entry_price)}원")
                            st.write(f"보유시간: {hours}시간 {minutes}분")
                            profit_color = "green" if current_profit >= 0 else "red"
                            st.markdown(f"현재 수익률: <span style='color:{profit_color}'>{current_profit:+.2f}%</span>", 
                                      unsafe_allow_html=True)
                            st.write(f"예상 잔고: {format_number(current_total)}원")
                        else:
                            st.markdown("### ⚪ 매수 대기")
                        st.write(f"현재 잔고: {format_number(current_balance)}원")
                    else:
                        st.markdown("### ⚪ 대기")
                        st.write(f"현재 잔고: {format_number(current_balance)}원")
                except Exception as e:
            ...
            with get_connection() as conn:
                ...
                try:
                    df_prices = pd.read_sql("""
                        ...
                        FROM price_data 
                        ORDER BY timestamp DESC LIMIT 360
                    """, conn)
                    ...
                    if not df_prices.empty:
                        ...
                        df_prices = df_prices[df_prices['timestamp'] >= six_hours_ago]
                                    ...
                                    if not sells.empty:
                                        fig.add_trace(
                                            go.Scatter(
                                                ...
                                                name='매도',text=[f"매도{' (손절)' if t == 'SELL_STOP' else ''}<br>{tm.strftime('%H:%M')}<br>{format_number(p)}원<br>{r:+.2f}%" 
                                                     for tm, p, r, t in zip(sells['timestamp'], sells['price'], sells['profit_rate'], sells['type'])],
            ...
            with get_connection() as conn:
                ...
                try:
                    ...
                    if not trades.empty:
                        ...
                        trades['수익률'] = trades.apply(
                            lambda x: f"{x['profit_rate']:+.2f}% {'(손절)' if x['sell_type'] == 'SELL_STOP' else ''}", 
                            axis=1
                        )
...
def main():
    ...
    while True:
        try:
            ...
            # 메인 컨텐츠 업데이트 (1분마다)
            if seconds_until_next_minute == 60:
                main_container.empty()  # 명시적으로 컨테이너를 비움
                with main_container:
                    render_dashboard()
