...
# 상승비율 정보 추가가
# 현재가, 매수가 가로 점선 추가
# MACD 히스토그램 width를 40000(40초)로 조정
# 데이터 업데이트 시점을 58초로 변경
# 10일치 데이터를 사용하도록 수정
            ...
            with get_connection() as conn:
                ...
                try:
                    ...
                    if not df_prices.empty and not df_market.empty:
                        ...
                        # 10일치 데이터 준비
                        df_prices = df_prices[df_prices['timestamp'] >= ten_days_ago]
                        df_market = df_market[df_market['timestamp'] >= ten_days_ago]
                            ...
                            if macd is not None and signal is not None:
                                ...
                                fig.add_trace(
                                    go.Scatter(
                                        ...
                                        y=df_prices['close'],
                                        name='BTC/KRW',
                                        line=dict(color='black', width=1.5),hovertemplate='%{y:,.0f}원'
                                    ),
                                    row=1, col=1,
                                    secondary_y=True
                                )
                                ...
                                """
                                trades = pd.read_sql(trades_query, conn, params=[ten_days_ago])
    ...
    while True:
        try:
            ...
            if now.second < 58:  # 58초부터 데이터 요청
                # 매 초마다 타이머 업데이트
                timer_text = f"다음 업데이트까지: **{60 - now.second}초**"
                title_container.markdown(f"""
                    # {title_text}
                    {timer_text}
                    {credits_text}
